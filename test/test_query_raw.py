...
def test_zip_nested_lsts():
    assert zip_nested_lists([['a', 'b']], [['y', 'z']]) == (
        [[('a', 'y'), ('b', 'z')]]
    )
    assert zip_nested_lists([['a', 'b'],['c', 'd']], [['w', 'x'],['y', 'z']]) == (
        [[('a', 'w'), ('b', 'x')], [('c', 'y'), ('d', 'z')]]
    )

def test_concat_fields():
    assert concat_fields([[['a', 'b']]]) == ['a', 'b']
    assert concat_fields([[['a', 'b']], [['y', 'z']], [['s', 't']]]) == (
        ['a,y,s', 'b,z,t']
    )

def test_parse_loc():
    loc_d2 = parse_loc('PR1.3')
    assert loc_d2['depth'] == 2
    assert loc_d2['seg'] == 'PR1'
    assert loc_d2['comp'] == 3 

    loc_d3 = parse_loc('DG1.3.1')
